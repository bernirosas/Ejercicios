...
class ProtagonistaDisfrazado: 
    ...
    def recuperar_casas_BFS_iterativo(self, casa_inicial: Casa)-> list:
        ...
        retornaresta lista creada [dic,dic,...,dic]'''
        visitados = set()
        cola = deque([(casa_inicial, 0)])
        lista_dic = []
        while len(cola) > 0:
            dic_actual = {}
            lista = cola.popleft()
            vertice = lista[0]
            dist_acumulada = lista[1]
            visitados.add(vertice)
            dic_actual["casa"] = vertice
            dic_actual["distancia"] = dist_acumulada
            lista_dic.append(dic_actual)
            if vertice.hijo_derecho:
                if vertice.hijo_derecho not in visitados:
                    cola.append((vertice.hijo_derecho, dist_acumulada + vertice.hijo_derecho.distancia))
            if vertice.hijo_izquierdo:
                if vertice.hijo_izquierdo not in visitados:
                    cola.append((vertice.hijo_izquierdo, dist_acumulada + vertice.hijo_izquierdo.distancia))
        return lista_dic
