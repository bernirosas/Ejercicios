...
from parametros import ACTUALIZAR_JUEGO, RESET_MARTILLO, TIEMPO_JUEGO

import parametros as p
from parametros import MARTILLO_D_X, MARTILLO_D_Y, MARTILLO_R_X, MARTILLO_R_Y, MARTILLO_U_X, MARTILLO_U_Y
from parametros import MARTILLO_L_X, MARTILLO_L_Y
...
class Martillo(QObject):
    ...
    def mover(self, dir):
        if dir == "L":
            self.pos_martillo.moveTo(MARTILLO_L_X, MARTILLO_L_Y)
        elif dir == "U":
            self.pos_martillo.moveTo(MARTILLO_U_X, MARTILLO_U_Y)
        elif dir == "R":
            self.pos_martillo.moveTo(MARTILLO_R_X, MARTILLO_R_Y)
        elif dir == "D":
            self.pos_martillo.moveTo(MARTILLO_D_X, MARTILLO_D_Y)
...
class LogicaJuego(QObject):
    ...
    def instanciar_timer(self):
        ...
        self.timer_martillo.setInterval(RESET_MARTILLO)
        self.timer_martillo.timeout.connect(self.reset_martillo)
        self.timers.extend([self.timer_actualizar_juego, self.timer_juego, self.timer_martillo])
